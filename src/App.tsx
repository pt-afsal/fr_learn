...
const activityMeta: Record<ActivityType, { icon: string; description: string }> = {
  ...
  'weekly-review': { icon: '↻', description: 'Review recent mistakes' },
}
    ...
    <div className="app-shell">
      ...
      <main className="main-area">
        <header className="mobile-header">
          ...
          <button className="secondary-button compact" onClick={() => setActiveView('settings')}>Settings</button>
        </header>
...
function Sidebar({ active, settings, onNavigate }: { active: string; settings: UserSettings; onNavigate: (id: (typeof navItems)[number]['id']) => void }) {
  return <aside className="sidebar">
    ...
    <div className="sidebar-footer"><span className="level-pill">{settings.currentLevel} → {settings.targetLevel}</span><small>{formatMinutes(settingsWeeklyMinutes(settings))} planned weekly</small></div>
  </aside>
}

function Notice({ text, onClose }: { text: string; onClose: () => void }) {
  return <div className="notice"><span>{text}</span><button onClick={onClose}>×</button></div>
}
...
const placementQuestions = [
  { prompt: 'Choose the correct sentence.', choices: ['Je suis étudiante.', 'Je est étudiante.', 'Je être étudiante.'], answer: 'Je suis étudiante.', level: 'A1' },
  { prompt: 'Complete: Nous ___ français.', choices: ['parlons', 'parlez', 'parlent'], answer: 'parlons', level: 'A1' },
  { prompt: 'Choose the correct article: ___ table.', choices: ['Le', 'La', 'Les'], answer: 'La', level: 'A1' },
  { prompt: 'Hier, elle ___ au cinéma.', choices: ['est allée', 'va', 'allait toujours'], answer: 'est allée', level: 'A2' },
  ...
  { prompt: 'Complete: Quand j’étais petite, je ___ souvent chez ma grand-mère.', choices: ['vais', 'allais', 'irai'], answer: 'allais', level: 'A2' },
  { prompt: 'Complete: Il faut que vous ___ à l’heure.', choices: ['venez', 'viendrez', 'veniez'], answer: 'veniez', level: 'B1' },
...
function VocabularyLibrary({ snapshot }: { snapshot: LearningSnapshot }) {
  const decks = snapshot.decks.filter((deck) => deck.level !== 'B2')
  ...
  return <div className="library-layout"><aside className="library-list"><h3>Decks</h3>{decks.map((item) => <button className={item.id === deck?.id ? 'selected' : ''} onClick={() => setSelectedId(item.id)} key={item.id}><span>{item.title}</span><small>{item.level}</small></button>)}</aside>{deck && <section className="lesson-panel"><span className="eyebrow">{deck.level} vocabulary</span><h2>{deck.title}</h2><p>{deck.description}</p><div className="word-grid">{cards.slice(0, 40).map((card) => <article key={card.id}><strong>{card.frontFr}</strong><span>{card.backEn}</span><small>{card.exampleFr}</small></article>)}</div></section>}</div>
}

function VerbLibrary({ snapshot }: { snapshot: LearningSnapshot }) {
  ...
  return <div className="library-layout"><aside className="library-list"><h3>Verbs</h3>{verbs.slice(0, 160).map((item) => <button className={item.id === verb?.id ? 'selected' : ''} onClick={() => setSelectedId(item.id)} key={item.id}><span>{item.infinitive}</span><small>{Math.round(average(Object.values(item.mastery)))}%</small></button>)}</aside>{verb && <section className="lesson-panel"><span className="eyebrow">{verb.level} · {verb.group}</span><h2>{verb.infinitive}</h2><p>{verb.meaning}</p>{Object.entries(verb.conjugations).slice(0, 5).map(([tense, forms]) => <div className="verb-table" key={tense}><h3>{humanTense(tense)}</h3>{Object.entries(forms).map(([person, form]) => <div key={person}><span>{person}</span><strong>{form}</strong></div>)}</div>)}</section>}</div>
}
...
function PracticeView({ snapshot, onRefresh, onOpen }: { snapshot: LearningSnapshot; onRefresh: () => Promise<void>; onOpen: (type: ActivityType, contentId?: string) => void }) {
  ...
  const [focus, setFocus] = useState('passé composé and imparfait')
  ...
  const cards: Array<{ type: ActivityType; title: string; description: string }> = [
    ...
    { type: 'vocabulary-review', title: 'Vocabulary review', description: 'Use spaced repetition with Again, Difficult, Good, and Easy.' },
    { type: 'conjugation', title: 'Conjugation drill', description: 'Practise verb forms by tense and subject pronoun.' },
...
function ProgressView({ snapshot, plan }: { snapshot: LearningSnapshot; plan: WeekPlan }) {
  ...
  return <>
    ...
    <section className="panel"><h2>Recent activity</h2>{snapshot.taskCompletions.slice(0, 12).length ? <div className="history-list">{snapshot.taskCompletions.slice(0, 12).map((item) => <div key={item.id}><span>✓</span><div><strong>{item.taskId.split('-').slice(3).join(' ')}</strong><small>{new Date(item.completedAt).toLocaleString()}</small></div>{item.score !== undefined && <b>{Math.round(item.score)}%</b>}</div>)}</div> : <p className="muted">Complete your first activity to start the history.</p>}</section>
  </>
}
...
function VocabularyWorkspace({ snapshot, contentId, onDone, onRefresh }: { snapshot: LearningSnapshot; contentId?: string; onDone: (score?: number) => Promise<void>; onRefresh: () => Promise<void> }) {
  ...
  const cards = (contentId ? snapshot.cards.filter((card) => card.deckId === contentId) : due.length ? due : snapshot.cards).slice(0, 10)
  const [index, setIndex] = useState(0); const [revealed, setRevealed] = useState(false); const [positive, setPositive] = useState(0)
  const card = cards[index]
  if (!card) return <EmptyState title="No flashcards found" description="There are no cards in this deck." />
  const rate = async (rating: 'again' | 'difficult' | 'good' | 'easy') => { await updateCardReview(card.id, rating); if (rating === 'good' || rating === 'easy') setPositive(positive + 1); await onRefresh(); if (index + 1 >= cards.length) void onDone(Math.round(((positive + (rating === 'good' || rating === 'easy' ? 1 : 0)) / cards.length) * 100)); else { setIndex(index + 1); setRevealed(false) } }
  return <div className="exercise flashcard-exercise"><span className="eyebrow">Card {index + 1} of {cards.length}</span><button className={`flashcard ${revealed ? 'revealed' : ''}`} onClick={() => setRevealed(true)}><strong>{card.frontFr}</strong>{revealed ? <><b>{card.backEn}</b><span>{card.exampleFr}</span><small>{card.exampleEn}</small></> : <small>Click to reveal</small>}</button>{revealed && <div className="rating-grid"><button onClick={() => void rate('again')}>Again</button><button onClick={() => void rate('difficult')}>Difficult</button><button onClick={() => void rate('good')}>Good</button><button onClick={() => void rate('easy')}>Easy</button></div>}</div>
}
...
function ConjugationWorkspace({ snapshot, contentId, onDone, onRefresh }: { snapshot: LearningSnapshot; contentId?: string; onDone: (score?: number) => Promise<void>; onRefresh: () => Promise<void> }) {
  const verbs = contentId ? snapshot.verbs.filter((verb) => verb.id === contentId) : [...snapshot.verbs].filter((verb) => verb.level !== 'B2').sort((a, b) => average(Object.values(a.mastery)) - average(Object.values(b.mastery))).slice(0, 20)
  const rounds = makeVerbRounds(verbs, 8)
  const [index, setIndex] = useState(0); const [answer, setAnswer] = useState(''); const [result, setResult] = useState<boolean | null>(null); const [score, setScore] = useState(0)
  ...
  const check = async () => { const correct = normalizeAnswer(answer) === normalizeAnswer(round.correct); setResult(correct); if (correct) setScore(score + 1); await updateVerbMastery(round.verb.id, round.tense, correct, answer, `${round.person} ${round.verb.infinitive}`); await onRefresh() }
  const next = () => { if (index + 1 >= rounds.length) void onDone(Math.round(score / rounds.length * 100)); else { setIndex(index + 1); setAnswer(''); setResult(null) } }
  return <div className="exercise"><span className="eyebrow">Verb {index + 1} of {rounds.length} · {humanTense(round.tense)}</span><h3>Conjugate <em>{round.verb.infinitive}</em></h3><p className="prompt-line">{round.person} <input value={answer} disabled={result !== null} onChange={(event) => setAnswer(event.target.value)} autoFocus /></p>{result !== null && <Feedback correct={result} answer={round.correct} explanation={`${round.person} ${round.correct}`} />}{result === null ? <button className="primary-button" disabled={!answer} onClick={() => void check()}>Check answer</button> : <button className="primary-button" onClick={next}>{index + 1 >= rounds.length ? 'Finish activity' : 'Next verb'}</button>}</div>
}
...
function PronunciationWorkspace({ snapshot, expectedText, onDone }: { snapshot: LearningSnapshot; expectedText?: string; onDone: (score?: number) => Promise<void> }) {
  const [expected, setExpected] = useState(expectedText ?? 'Je voudrais prendre rendez-vous jeudi après-midi.')
  ...
  const record = () => {
    ...
    recognition.onerror = () => { setListening(false); setError('The browser could not capture the phrase. Try again or type the transcription manually.') }
    recognition.onresult = (event: SpeechRecognitionEventLike) => setRecognized(event.results[0][0].transcript)
    recognition.start()
  }
...
function humanTense(value: string) { return ({ present: 'Présent', passeCompose: 'Passé composé', imparfait: 'Imparfait', futurSimple: 'Futur simple', subjunctive: 'Subjonctif' } as Record<string, string>)[value] ?? value }
...
interface RecognitionLike { lang: string; interimResults: boolean; maxAlternatives: number; onstart: (() => void) | null; onend: (() => void) | null; onerror: (() => void) | null; onresult: ((event: SpeechRecognitionEventLike) => void) | null; start: () => void }
